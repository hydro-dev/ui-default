...
let translate = function (language, messageKey, messageParams) {
  ...
  if (messageParams) {
    for (const param of messageParams) message = message.replace('%s', param);
  }
  return message;
};
...
function guessMediaType(url) {
  ...
  if (validVideoExtensions.indexOf(extension.toLowerCase()) !== -1) return 'video';
  return 'image';
}
...
function tokenizeImagesAndMedia(state, silent, md) {
  let attrs; let code; let label; let pos; let ref; let res; let title; let tokens; let start;
  let href = '';
  ...
  if (pos < max && state.src.charCodeAt(pos) === 0x28) { // Parenthesis: (
    ...
    for (; pos < max; pos++) {
      ...
      if (!md.utils.isSpace(code) && code !== 0x0A) break; // LF \n
    }
